...
func GetCollection(CollectionID string) (collection.Collection, error) {
	...
	for rows1.Next() {
		...
		user.Auther.UserID, _ = uuid.Parse(collection_user_id_tmp)
	}
	//ここからUserID取得
	rows2, err := db.Query("SELECT * FROM user WHERE user_id = ?", collection_user_id_tmp)
	if err != nil {
		log.Fatal(err)
	}
	var name_tmp string
	...
	for rows3.Next() {
		...
		question_tmp, qerr := GetQuestion(idtemp)
		if qerr != nil {
			return user, nil
		}
